...
def box_prompt(masks, bbox, target_height, target_width):
    ...
    w = w = masks[0]["segmentation"].shape[1]
    ...
    masks_tensor = torch.tensor([mask['segmentation'] for mask in masks])
    masks_area = torch.sum(masks_tensor[:, bbox[1] : bbox[3], bbox[0] : bbox[2]], dim=(1, 2))
    orig_masks_area = torch.sum(masks_tensor, dim=(1, 2))

    union = bbox_area + orig_masks_area - masks_area
    IoUs = masks_area / union
    max_iou_index = torch.argmax(IoUs)

    return masks_tensor[max_iou_index].cpu().numpy(), max_iou_index
